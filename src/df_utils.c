...
int *df_utils_serial_df(int *integers, int max_number, int num_integers) {
  int primes_mask_size;
  bool *primes_mask =
      df_utils_sieve_of_eratosthenes(max_number, &primes_mask_size);
  int num_primes;
  int *primes =
      mask_vector_to_int_vector(primes_mask, primes_mask_size, &num_primes);
  int *integers_num_divisors = malloc(num_integers * sizeof(int));
  // not saving directly just to profile only the computation
  for (int i = 0; i < num_integers; i++) {
    integers_num_divisors[i] = df_utils_df(integers[i], primes, num_primes);
#ifdef DEBUG
